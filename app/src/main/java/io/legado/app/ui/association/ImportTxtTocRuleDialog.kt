...
/**
 * 导入 TXT 目录规则（Compose 版，沿用 BatchImportDialog 规范）。
 */
class ImportTxtTocRuleDialog() : BaseComposeDialogFragment() {
    ...
    override fun Content() {
        ImportAssociationContent(
            ...
            itemTitle = { it.name },
            itemSubtitle = { it.chapterRule.takeIf { rule -> rule.isNotBlank() } },
            onUpdateItem = { index, data ->
                (data as? TxtTocRule)?.let { viewModel.allSources[index] = it }
            },
