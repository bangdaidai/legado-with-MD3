 ...
 * 文本选择浮层共用的进出动画规格。
 *
 * [TextActionSelectionMenu] 及其子菜单共用同一套时长和缩放比例，切换不同层级时
 * 避免出现两套节奏叠加的割裂感。各菜单只传自己的缩放原点：
 * 居中的小菜单用 [TransformOrigin.Center]，贴着选区展开的用朝向选区的那条边。
 */
internal object SelectionMenuMotion {
     ...
     * 出场总时长，取淡出与缩放中较长的一个。
     *
     * 主动关闭浮层时要按它来决定延迟多久才真正摘掉浮层：早于该时长会把出场动画截断。
     */
