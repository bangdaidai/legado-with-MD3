...
## 主题色（切换颜色）
在分享卡片预览面板，**点击左上角切换颜色按钮**可弹出色板：默认 / 8 种预设色 / 自定义取色。选中后**整套配色**（背景 / 表面 / 文字 / 强调色）会一起变。颜色仅本次预览生效，不写库、不修改模板。
...
暗色方案下，渲染器会给 `<html>` 加上 `class="bp-dark"`。**语义色**（比如「在读=绿、已读=蓝」这种不该跟着主色变的信息色）就用它翻色：
...
改不改都行——不改的模板不会坏，只是选色对它没效果。想固定不参与换色的元素（比如封面图圆角、语义状态色）**保留原色**即可。
