...
/**
 * 导入字典规则（Compose 版，沿用 BatchImportDialog 规范）。
 */
class ImportDictRuleDialog() : BaseComposeDialogFragment() {
    ...
    override fun Content() {
        ImportAssociationContent(
            ...
            itemTitle = { it.name },
            itemSubtitle = { it.urlRule.takeIf { url -> url.isNotBlank() } },
            onUpdateItem = { index, data ->
                (data as? DictRule)?.let { viewModel.allSources[index] = it }
            },
